...
- **Never use em dashes in UI copy** — anything a visitor reads (page text, project descriptions, alt text, metadata, button labels). No em dashes, ever: use commas, periods, colons, semicolons, or parentheses, and reword or repunctuate to keep it grammatical. The rule is scoped to shipped copy — this file, code comments, and commit messages are exempt and use em dashes freely.
- **Copy in `projects.ts` carries inline markup, and it is a fixed grammar, not markdown.** `InlineMarkup` in `src/app/projects/[slug]/ProjectContent.tsx` renders exactly three spans — `` `backticks` `` → `<code>`, `_underscores_` → `<em>` (film and book titles), `[text](href)` → an external link (`target="_blank"`, `rel="noopener noreferrer"`, the `link` recipe). A blank line (`\n\n`) is a paragraph break: `description`, `why` and `techNotes` all render through `Paragraphs`, one `<p>` per chunk. Nothing else parses — headings, lists and bold are not supported, so don't write them. The grammar lives in `src/app/projects/[slug]/markup.ts` and has **two halves that must stay in sync**: `markupSpan`/`linkSpan` render it, and `stripMarkup` flattens it back to plain text for `<meta name="description">` in `page.tsx`. Add a span type to one half and you must add it to the other, or raw syntax leaks into metadata.
...
- **Recipes for repeated primitives**: `button` (`visual: solid|outline`, `size: sm|md`), `card` (`interactive: true`), `tag`, `link` — import from `styled-system/recipes`. Route one-offs stay plain `css()`. Combine a recipe with extra styles via `cx(card({...}), css({...}))`; the `utilities` layer wins over `recipes`, so `css()` overrides land. That layer trick works **only** across layers: two `css()` calls land in the *same* layer, so `cx(css(a), css(b))` resolves by stylesheet order, not argument order, and an override may silently lose. To compose styles that share a property, merge the objects instead — `css.raw()` for the base, then `css(base, { ...overrides })` at each use. The prose styles at the top of `ProjectContent.tsx` are the worked example: one `proseBase`, with `closingNote` overriding its `color`.
