...
- **Neutral ramp steps are shared across themes** (`neutral.900` is light text _and_ near the dark bg; `neutral.200` is dark text _and_ near the light bg). Never retune a theme by editing a step's lightness without checking both themes' consumers in `semanticTokens` — remap the semantic token instead, and keep the ramp monotonic. Dark `bg`/`surface`/`border` intentionally sit one step lighter than their names suggest (900/800/700), and light `border` is `neutral.300`.
