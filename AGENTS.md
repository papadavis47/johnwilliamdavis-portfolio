...
- **Colors are semantic tokens only.** Never reach for a raw ramp step (`accent.600`, `neutral.200`) in a component — use `bg`, `surface`, `text`, `text.muted`, `border`, `accent`, `accent.hover`, `accent.emphasis`, `accent.fg`, `accent.subtle`. All ramps are oklch; the neutrals are driven by hue constants at the top of `panda.config.ts` (`LIGHT_HUE` · `DARK_HUE`), and every `accent` step shares hue `272`.
